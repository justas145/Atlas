...
def create_experience_doc(
    console_output, model_name="llama3-70b-8192", temperature=0.3
):
    ...
    if "gpt" in model_name:
        ...
        llm = (
            ChatGroq(temperature=temperature, model_name=model_name, api_key=api_key)
            | StrOutputParser()
        )
    ...
    dos_donts_list = llm.invoke(
        do_and_dont_list_prompt.format(log=processed_log_commands)
    )
    ...
    relative_values_dos_donts_list = llm.invoke(
        relative_values_dos_donts_list_prompt.format(
            dos_donts_list=dos_donts_list_transformation
        )
    )
